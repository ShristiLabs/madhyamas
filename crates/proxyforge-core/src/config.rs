...
impl ProxyConfig {
    ...
    /// Priority:
    /// 1. PROXYFORGE_PUBLIC_IP environment variable (for Docker/container environments)
    /// 2. PROXYFORGE_HOST_IP environment variable (alternative for Docker)
    ...
    pub fn detect_private_ip() -> Option<String> {
        ...
        if let Ok(ip) = std::env::var("PROXYFORGE_PUBLIC_IP") {
            if !ip.is_empty() {
                return Some(ip);
            }
        }
        if let Ok(ip) = std::env::var("PROXYFORGE_HOST_IP") {
            if !ip.is_empty() {
                return Some(ip);
            }
        }
