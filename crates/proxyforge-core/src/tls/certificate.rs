...
impl CertificateManager {
    ...
    const ORG_NAME: &'static str = "ProxyForge";
    ...
    pub async fn new(cert_path: &str) -> crate::Result<Arc<Self>> {
        ...
        let ca_cert_path = Path::new(cert_path).join("proxyforge-ca.pem");
        let ca_key_path = Path::new(cert_path).join("proxyforge-ca-key.pem");
    ...
    async fn load_ca(
        ...
        dn.push(DnType::CommonName, "ProxyForge Root CA");
