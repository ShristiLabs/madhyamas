//! PostgreSQL-backed [`ConfigStoreBackend`] implementation.
//!
//! [`PostgresConfigStore`] wraps a [`sqlx::PgPool`] and persists
//! configuration key/value pairs as JSON text in a single `config` table,
//! mirroring the schema and serialization used by the SQLite backend. All
//! queries use runtime SQL strings with `$N` placeholders.
...
#[async_trait]
impl ConfigStoreBackend for PostgresConfigStore {
    async fn get_value(&self, key: &str) -> Result<Option<serde_json::Value>> {
        let row: Option<ConfigRow> =
            sqlx::query_as::<_, ConfigRow>("SELECT value FROM config WHERE key = $1")
                .bind(key)
