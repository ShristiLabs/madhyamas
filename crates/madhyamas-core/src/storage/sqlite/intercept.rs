//! SQLite-backed [`InterceptStoreBackend`] implementation.
//!
//! [`SqliteInterceptStore`] wraps a [`sqlx::SqlitePool`] and persists
//! intercept rules (mocks, rewrites, breakpoints, throttle profile, block
//! list entries) in five SQLite tables, mirroring the schema and JSON
//! serialization used by the former `rusqlite` `InterceptStore`. All
//! queries use runtime SQL strings with `?` placeholders.
...
pub struct SqliteInterceptStore {
    pool: SqlitePool,
}

impl SqliteInterceptStore {
...
impl InterceptStoreBackend for SqliteInterceptStore {
    ...
    async fn delete_breakpoint_rule(&self, id: &str) -> Result<bool> {
        let result = sqlx::query("DELETE FROM breakpoint_rules WHERE id = ?")
            .bind(id)
            .execute(&self.pool)
            .await?;
        Ok(result.rows_affected() > 0)
    ...
    async fn delete_block_list_entry(&self, id: &str) -> Result<bool> {
        let result = sqlx::query("DELETE FROM block_list_entries WHERE id = ?")
            .bind(id)
            .execute(&self.pool)
            .await?;
        Ok(result.rows_affected() > 0)
