...
| Gateway capability scoping (analysis) | [docs/GATEWAY_SCOPING.md](docs/GATEWAY_SCOPING.md) | Decision document (issue #85): which API-gateway behaviors Madhyamas should adopt ("gateway behaviors you can debug"); per-feature verdicts, rejected features, follow-up issues |
| Per-user traffic scoping (analysis) | [docs/TRAFFIC_SCOPING_PER_USER.md](docs/TRAFFIC_SCOPING_PER_USER.md) | Roadmap analysis: options for per-user traffic visibility on shared Enterprise instances (session ownership vs per-entry attribution vs instance-per-user), tradeoff matrix, staged recommendation, open questions |
| Per-app traffic scoping (analysis) | [docs/TRAFFIC_SCOPING_PER_APP.md](docs/TRAFFIC_SCOPING_PER_APP.md) | Roadmap analysis: why per-app identity is inference at the proxy (signals inventory), options (whitelist recording, entry metadata client_addr/listener/SNI, per-app VPN routing, dedicated listeners), tradeoff matrix, phased recommendation |
| Per-device scoping & identity (analysis) | [docs/TRAFFIC_SCOPING_PER_DEVICE.md](docs/TRAFFIC_SCOPING_PER_DEVICE.md) | Device identity via per-device proxy credentials (identity travels in-band); steering modes (manual proxy config vs companion VPN with credential injection); Android/iOS capability matrix (VpnService vs NEPacketTunnelProvider, per-app routing, memory caps); rejected IP/MAC out-of-band correlation |
| Credential onboarding flow (analysis) | [docs/CREDENTIAL_ONBOARDING.md](docs/CREDENTIAL_ONBOARDING.md) | QR-based device credential onboarding and device-derived agent keys (Enterprise-only flow; OSS unchanged): step-by-step exists/gap analysis, credential hierarchy (agent keys bound to parent device, cascade lifecycle), two-axis enforcement (device binding × user-selected feature scopes with taxonomy and MCP tool filtering), tier placement, device-scoped intercept rules, pipeline attribution context, QR payload + enrollment tokens, platform apply paths incl. stock-Android limits, plaintext-listener security gap, component map and phasing |
| Cert-pinning overrides (analysis) | [docs/CERT_PINNING_OVERRIDES.md](docs/CERT_PINNING_OVERRIDES.md) | Roadmap analysis: per-host certificate overrides for pinned apps when the operator holds the pinned identity's key; pin-type/key-possession matrix, options (override / CA substitution / app-side bypasses), design sketch, security constraints |
